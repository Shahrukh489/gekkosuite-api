...
public class UserRepository : BaseRepository, IUserRepository
{
    ...
    public Task<IEnumerable<MembershipEntity>> GetUserOrganizationMembershipsAsync(Guid organizationId, Guid userId)
    {
        const string sql = """
            ...
            GROUP BY m.membership_id, o.organization_id, o.name, r.role_id, r.name, ma.assigned_at, ma.expires_at
            """;
    ...
    public Task<IEnumerable<MembershipEntity>> GetUserStoreMembershipsAsync(Guid organizationId, Guid userId)
    {
        const string sql = """
            ...
            WHERE m.user_id = @userId
              ...
              AND m.is_active AND NOT m.is_deleted
              AND (ma.expires_at IS NULL OR ma.expires_at > now())
            GROUP BY m.membership_id, s.store_id, s.name, r.role_id, r.name, ma.assigned_at, ma.expires_at, m.created_at
    ...
    public Task<IEnumerable<MembershipEntity>> GetUserStoreMembershipsByStoreIdAsync(Guid organizationId, Guid userId, Guid storeId)
    {
        const string sql = """
            ...
            WHERE m.user_id = @userId
              ...
              AND m.is_active AND NOT m.is_deleted
              AND (ma.expires_at IS NULL OR ma.expires_at > now())
            GROUP BY m.membership_id, s.store_id, s.name, r.role_id, r.name, ma.assigned_at, ma.expires_at
            """;
