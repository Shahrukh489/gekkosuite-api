...
public interface IUserService
{
    ...
    /// </summary>
    Task<MembershipEntity?> GetOrgMembershipAsync(Guid organizationId, Guid userId);

    /// <summary>
    /// Returns the user's live STORE memberships (each with its store name and role), oldest-first.
    /// </summary>
    Task<IEnumerable<MembershipEntity>> GetStoreMembershipsAsync(Guid organizationId, Guid userId);
